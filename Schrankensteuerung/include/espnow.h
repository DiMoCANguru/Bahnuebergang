...
you will need ot ge tits MAC via WiFi.softAPmacAddress()
*/
...
/*  Serial.print("R- ");
  for (byte i=0; i<CAN_ENCAP_SIZE; i++){
    Serial.print(opFrame[i], HEX);
    Serial.print("-");
  }*/
  switch (opFrame[0x01]){
    case BlinkAlive:
...
void sendTheData() {
  esp_now_send(master.peer_addr, opFrame, CAN_ENCAP_SIZE);
}
...
void addMaster() {
  ...
  //Add the remote master node to this slave node
	if( esp_now_add_peer(masterNode) == ESP_OK) {
		DEBUG_PRINTLN("Master Added As Peer!");
	}
}
